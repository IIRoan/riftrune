...
import { DeckReadOnlyError, DeckService } from '../../src/services/deck-service.js';
...
describe('DeckService.listForUser', () => {
  test('filters owned decks by name, description, legend, and champion', async () => {
    ...
    const byLegend = await service.listForUser('user-1', { q: 'jinx', source: 'owned' });
    expect(byLegend.items.map((deck) => deck.id).sort()).toEqual(['deck_a', 'deck_c']);

    const byChampion = await service.listForUser('user-1', { q: 'viktor', source: 'owned' });
    expect(byChampion.items.map((deck) => deck.id)).toEqual(['deck_b']);

    const emptyQuery = await service.listForUser('user-1', { q: '   ', source: 'owned' });
    expect(emptyQuery.items).toHaveLength(3);
  });

  test('marks owned decks as editable', async () => {
    const service = new DeckService(createListMockDb([ownedPayload('deck_owned', 'Mine')]));
